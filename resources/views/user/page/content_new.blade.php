    ...
    <main>
        <div class="container">
            <div class="title-page pt-md-4 pt-2 pb-2 text-success">
                ...
                <a href="{{ route('home') }}">Trang chủ</a> / <a href="{{ route('list_project', ['category' =>$blogs->id_category ]) }}">{{ \App\Helpers\ConstCommon::getnameByIDCategory($blogs->id_category) }}</a> / {{ $blogs->name }}
            </div>
            <div class="bg-white new-card my-2 content-new">
                <img src="{{ \App\Helpers\ConstCommon::getLinkImageToStorage($blogs->img) }}" alt="">
                <div class="px-2">
                    <div class="new-type pt-2 ">{{ \App\Helpers\ConstCommon::getnameByIDCategory($blogs->id_category) }}</div>
